...
def leading_comment(node):
    ...
    for t in node.getFirstToken().trivia:
        if t.kind in (pyslang.parsing.TriviaKind.BlockComment, pyslang.parsing.TriviaKind.LineComment):
            return t.getRawText()
    return None
...
def main():
    ...
    root = tree.root
    mod = next(m for m in root.members if m.kind == pyslang.syntax.SyntaxKind.ModuleDeclaration)
    header = mod.header
    ...
    print("\n-- parameters --")
    decls = [d for d in header.parameters.declarations
    ...
    for i, d in enumerate(decls):
        ...
        trailing = None
        for t in next_node.getFirstToken().trivia if hasattr(next_node, "getFirstToken") else next_node.trivia:
    ...
    print("\n-- ports --")
    ports = [p for p in header.ports.ports
             if p.kind != pyslang.syntax.SyntaxKind.Unknown]
