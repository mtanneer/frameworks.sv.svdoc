...
def main(argv=None):
    ...
    Parses CLI arguments and either scaffolds missing doc comments (``--fix``),
    or documents the given file, optionally resolving cross-file types against
    ``more_files`` and writing Markdown/HTML output (``--out``).

    :param argv: Argument list to parse, as passed to :func:`argparse.ArgumentParser.parse_args`.
