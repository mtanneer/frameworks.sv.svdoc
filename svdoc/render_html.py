...
def _params_section(params) -> list:
    if not params:
        return []
    lines = ["<h2>Parameters</h2>", "<table><tr><th>Name</th><th>Type</th><th>Default</th><th>Description</th></tr>"]
    for p in params:
        lines.append(
...
def _ports_section(ports) -> list:
    if not ports:
        return []
    lines = ["<h2>Ports</h2>", "<table><tr><th>Name</th><th>Direction</th><th>Type</th><th>Description</th></tr>"]
    for p in ports:
        lines.append(
            f"<tr><td><code>{escape(p.name)}</code></td>"
...
def render_interface(iface: InterfaceDoc) -> str:
    ...
    if iface.signals:
        lines += ["<h2>Signals</h2>", "<table><tr><th>Name</th><th>Type</th><th>Description</th></tr>"]
    ...
    if iface.modports:
        ...
        for mp in iface.modports:
            lines.append(f'<h3 id="{escape(mp.name)}"><code>{escape(mp.name)}</code></h3>')
            if mp.doc:
                lines.append(f"<p>{escape(mp.doc)}</p>")
            lines.append("<table><tr><th>Direction</th><th>Signals</th><th>Description</th></tr>")
            for g in mp.port_groups:
                signals = ", ".join(f"<code>{escape(s)}</code>" for s in g.signals)
                lines.append(f"<tr><td>{escape(g.direction)}</td><td>{signals}</td><td>{escape(g.doc or '')}</td></tr>")
            lines.append("</table>")
...
def render_package(pkg: PackageDoc) -> str:
    ...
    for t in pkg.typedefs:
        ...
        if t.kind == "enum":
            lines.append(f"<p>Enum (<code>{escape(t.base_type)}</code>)</p>" if t.base_type else "<p>Enum</p>")
            lines.append("<table><tr><th>Value</th><th>Number</th><th>Description</th></tr>")
            for v in t.values:
                lines.append(f"<tr><td><code>{escape(v.name)}</code></td><td>{escape(v.value or '')}</td><td>{escape(v.doc or '')}</td></tr>")
            lines.append("</table>")
        elif t.kind == "struct":
            lines.append("<p>Packed struct</p>")
            lines.append("<table><tr><th>Field</th><th>Type</th><th>Description</th></tr>")
            for f in t.fields:
                lines.append(f"<tr><td><code>{escape(f.name)}</code></td><td><code>{escape(f.type)}</code></td><td>{escape(f.doc or '')}</td></tr>")
            lines.append("</table>")
        else:
    ...
    for s in pkg.subroutines:
        ...
        header = f"Function returning <code>{escape(s.return_type)}</code>" if s.kind == "function" else "Task"
        lines.append(f"<p>{header}</p>")
        if s.args:
            lines.append("<table><tr><th>Name</th><th>Direction</th><th>Type</th><th>Description</th></tr>")
            for a in s.args:
                lines.append(
